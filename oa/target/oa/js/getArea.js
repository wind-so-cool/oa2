...
function GetArea(){
	...
	this.getProvinces = function(chId,provinceId,eprovinceId,ecityId){
		...
		if (len <= 1) {
			var areaId = chId;
			$.getJSON("areaController/getAreaListByParentId/"+areaId,function(data) {
				var pObj = $("#"+eprovinceId);
                pObj.append("<option value=''>请选择:</option>");
                $.each(data.data.areaList, function(keyi, items) {
	...
	this.getCity = function(provinceId,cityId,ecityId,ecountyId){
		...
		if(provinceId){
			if (len <= 1) {
				$.getJSON("areaController/getAreaListByParentId/"+provinceId, function(data) {
					$.each(data.data.areaList, function(keyi, items) {
    ...
    this.getCounty = function(cityId,countyId,ecountyId){
		...
		if(cityId){
			if (len <= 1) {
				$.getJSON("areaController/getAreaListByParentId/"+cityId, function(data) {
					$.each(data.data.areaList, function(keyi, items) {
